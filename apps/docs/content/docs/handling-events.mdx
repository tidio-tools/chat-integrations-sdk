...
`onMessageUpdated` fires when a user edits a message, and `onMessageDeleted` when one is removed. Both are lifecycle events: they never route through `onNewMessage`, `onNewMention`, or `onSubscribedMessage`, and the concurrency strategies do not apply to them. Slack sends them for `message_changed` / `message_deleted`; Teams for `messageUpdate` (edits and undeletes) / `messageDelete` (soft deletes, which carry no `previousMessage`).
...
`onMemberJoinedChannel` fires when a user joins a channel - including the bot itself. Slack sends it for `member_joined_channel`; Teams sends it for `conversationUpdate` members added, which covers the bot being installed in a personal chat, group chat, or team. Use it to post welcome messages or onboard users automatically.
...
| `raw` | `unknown` (optional) | The platform payload (Slack event, Teams `conversationUpdate` activity) |
