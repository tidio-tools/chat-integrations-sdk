...
Teams sends a `conversationUpdate` activity with `membersAdded` when users join a team or group chat and when the bot itself is added - in personal, group chat, and team scope alike. The adapter dispatches each added member as an `onMemberJoinedChannel` event; compare `event.userId` with `adapter.botUserId` (`28:<app id>`) to react to your own installation:
...
bot.onMemberJoinedChannel(async (event) => {
  ...
  await event.adapter.postMessage(
    event.channelId,
    "Hi! Mention me in a channel or message me here to get started."
  );
});
```

`installationUpdate` is not mapped on purpose: Teams fires it alongside the `conversationUpdate` above on install, so handling both would double-post.

### Edits and deletes

Teams sends `messageUpdate` when a message is edited or a soft-deleted message is restored, and `messageDelete` when one is soft-deleted. The adapter dispatches them as `onMessageUpdated` (with the full replacement message) and `onMessageDeleted`. Deletes identify the message by id only - Teams does not include the removed body, so `previousMessage` is never set.
